...
def determine_source_states(adata, embedding_key, graph_dist=True, n_neighbors=30, 
    ...
    By setting cell i as start cell, the pseudotime :math:`T(x_i) = {t_1, t_2, ..., t_n}` can be computed, and 
    the empirical distribution can be divided into :math:`m` portions that :math:`X_1, X_2, ..., X_m`, according to time :math:`T(x_i)`. And the 
    transport cost for cell i can be defined by optimal transport in the graph :math:`G` that
    
    .. math::

        L(X,T(x_i))=\sum_{i=1}^{m-1}\inf_{\pi}\sum_{x \in X_i}\sum_{y \in X_{i+1}}c(x,y | G)\pi(x,y)

    where :math:`c(x,y|G)` is the shorest path distance between two cells :math:`x,y` in graph :math:`G`.
    Due to the property of diversity differentiation, the defined transport cost of real source cell will be smallest, that

    .. math::

        {source}^* = arg \min_{x_i \in X}L(X, T(x_i))
    ...
    .. note::
        ...
        will detect linear progression and compute cytotrace score with very low weight (avaid choosing progressing cell) to choose the real source cell.
    ...
    n_comps: `int` (default: 15)
        Number of diffmap components, which is used for pseudotime computation
