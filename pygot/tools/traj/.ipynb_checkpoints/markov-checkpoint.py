...
def velocity_graph(data, embedding_key, velocity_key,  split_negative=True, copy=False, graph=None, kernel='cosine', D=1., backward=False):
    ...
    P = _transition_matrix(adata, embedding_key, velocity_key,  graph, method=kernel, norm=False, D=D, backward=backward)
    if split_negative:
...
def _transition_matrix(adata, embedding_key, velocity_key, graph, method='inner', norm=False, D=1., backward=False):
    vt = adata.obsm[velocity_key]
    vt = vt if backward == False else -vt
    neighbors = find_neighbors(graph)
    rows, cols, data = [], [], []
    if method=='inner':
        func = partial(inner_kernel, D=D)
    elif method == 'cosine':
        func = cosine
    else:
