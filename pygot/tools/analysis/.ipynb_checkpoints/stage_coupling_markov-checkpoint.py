...
class CellFate:
    """Cell fate prediction based on the velocity graph.

    Based on the velocity graph, the cell fate prediction is implemented by the following steps: 
    
    1. Compute the velocity graph based on the velocity field, using pygot.tl.traj.velocity_graph. 
    
    2. Compute the transition probability matrix based on the positive velocity graph. 
    
    3. Aggreate the target cells as recurrent states. 
    
    4. Solve absorbing probabilities for the recurrent states as cell fate prediction. 
    ...
    def fit(
        ...
        mutual: `bool`
            Whether to use mutual nearest neighbors graph
...
class TimeSeriesRoadmap:
    ...
    Based on the velocity graph, the developmental tree inference is implemented by the following steps: 
    
    1. Compute the velocity graph between two adjacent time points based on the velocity field, using pygot.tl.traj.velocity_graph. 
    
    2. Compute the forward and backward transition probability matrix based on the positive velocity graph. 
    
    3. Compute the absorbing probabilities for the cell types in the next and previous timepoint.
    
    4. Compute forward and backward state coupling matrix and aggregate them into a state coupling matrix by hadamard product.
    
    5. Compute the null distribution for the state coupling matrix 
