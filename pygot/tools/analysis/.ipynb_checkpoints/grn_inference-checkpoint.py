...
class GRN:
    ...
    The regulatory gene will regulate the expression of target gene by increase or decrease gene velocity.
    Due to that, the regulatory relationship model as 
    
    .. math::
        v(x_i) = \sum_{j}{G_{j,i}x_j} - \\beta_i x_i, \quad \\beta_i > 0

    Here :math:`G_{j,i}` represents gene j regulate gene i, and :math:`\\beta_i` represents degrade rate of gene i.
    Besides, the regulatory network is sparse, so the l1 regularization item is added to fit the model.
    The final solution is

    .. math::
        G^*, \\beta^* = arg \min_{G, \\beta}{\Vert Gx-\\beta x - v(x) \Vert}^2 - \sum_i{\lambda \Vert G_{\dot, i} \Vert^1}, \\\\
        s.t. diag(G) = 0, \\\\
        \\beta >= m >= 0
